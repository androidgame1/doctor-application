...
<head>
    ...
    <title>{{__('messages.activity')}}</title>
    <style>
        @page {
             margin: 0px;
             padding:0px;
...
<body>
    <header class="header">
        ...
        <div class="div-header-title">
            <span class="header-title">{{__('messages.activity')}} : <span class="span-series">N° {{$activity->series}}</span></span>
        </div>
    </header>
    <footer class="footer">
    ...
    <main>
                ...
                <tbody>
                    <tr>
                        <td width="50%">
                            ...
                            <br>
                            <div><b>{{__('messages.fullname')}} : {{$activity->patient->fullname}}</b></div>
                            <div><b>{{__('messages.email')}} : </b>{{$activity->patient->email}}</div>
                            <div><b>{{__('messages.address')}} : </b>{{$activity->patient->address}}</div>
                            <div><b>{{__('messages.phone')}} : </b>{{$activity->patient->email}}</div>
                        </td>
                        <td width="15%" class="text-verticaly-top">
                            <div><div><b>{{__('messages.date')}} : </b>{{\Carbon\Carbon::parse($activity->start_date)->format('d/m/Y')}}</div></div>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div class="div-table">
            <table class="table-sale-invoice">
                ...
                <tbody>
                    @foreach($activity->activity_lines as $line)
                        <tr>
                            <td>
