...
<div class="row page-titles">
    <div class="col-md-5 align-self-center">
        <h4 class="text-themecolor">{{__('messages.activity_payments')}}</h4>
    </div>
                            ...
                            <table class="table table-bordered table-striped table-datatable">
                                ...
                                <tbody>
                                    @foreach($activity_payments as $value)
                                        <tr>
                                            ...
                                            <td>{{$value->activity->ht_total_amount}} <b>MAD</b></td>
                                            ...
                                            <td>
                                                ...
                                                <!-- <a href="@if(auth()->user()->is_administrator){{route('administrator.activity_payment.edit',$value->id)}}@else javascript:void(0) @endif" data-toggle="tooltip" title="{{__('messages.edit')}}"> <i class="fa fa-pencil text-success m-r-10 icon-datatable"></i> </a> -->
                                                <a href="javascript:void(0)" class="btn-destroy-item" data-toggle="modal" data-target="#div-destroy-old-item" data-url-destroy="@if(auth()->user()->is_administrator){{route('administrator.activity_payment.destroy',$value->id)}}@else javascript:void(0) @endif" data-title="{{__('messages.delete_activity_payment')}}" data-message="{{__('messages.do_you_want_to_delete_this')}} {{__('messages.activity_payment')}} ?" data-toggle="tooltip" title="{{__('messages.delete')}}"> <i class="fa fa-trash text-danger icon-datatable"></i> </a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                                <tfoot>
                                    <tr>
                                        ...
                                        <td colspan="6"><span class="text-success font-bold">{{$activity->given_total_amount}} <b>MAD</b></span></td>
                                    </tr>
                                    <tr>
                                        <td><b>{{__('messages.total_remaining_amount')}}</b></td>
                                        <td colspan="6"><span class="text-danger font-bold">{{$activity->remaining_total_amount}} <b>MAD</b></span></td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
