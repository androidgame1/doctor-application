                                ...
                                <tbody>
                                    @foreach($acts as $value)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="javascript:void(0)" data-url-show="@if(auth()->user()->is_administrator){{route('administrator.act.show',$value->id)}}@else javascript:void(0) @endif" class="btn-show-act" data-toggle="modal" data-target="#div-show-old-act" title="{{__('messages.show')}}"> <i class="fa fa-eye text-info m-r-10 icon-datatable"></i> </a>
                                                <a href="javascript:void(0)" data-url-edit="@if(auth()->user()->is_administrator){{route('administrator.act.edit',$value->id)}}@else javascript:void(0) @endif" class="btn-edit-act" data-url-update="@if(auth()->user()->is_administrator){{route('administrator.act.update',$value->id)}}@else javascript:void(0) @endif" data-toggle="modal" data-target="#div-edit-old-act" title="{{__('messages.edit')}}"> <i class="fa fa-pencil text-success m-r-10 icon-datatable"></i> </a>
                                                <a href="javascript:void(0)" class="btn-destroy-item" data-toggle="modal" data-target="#div-destroy-old-item" data-url-destroy="@if(auth()->user()->is_administrator){{route('administrator.act.destroy',$value->id)}}@else javascript:void(0) @endif" data-title="{{__('messages.delete_act')}}" data-message="{{__('messages.do_you_want_to_delete_this')}} {{__('messages.act')}} ?" title="{{__('messages.delete')}}"> <i class="fa fa-close text-danger icon-datatable"></i> </a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
