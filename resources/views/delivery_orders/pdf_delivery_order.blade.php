...
<head>
    ...
    <title>{{__('messages.delivery_order')}}</title>
    <style>
        @page {
             margin: 0px;
             padding:0px;
...
<body>
    <header class="header">
        ...
        <div class="div-header-title">
            <span class="header-title">{{__('messages.delivery_order')}} : <span class="span-series">N° {{$delivery_order->series}}</span></span>
        </div>
    </header>
    <footer class="footer">
    ...
    <main>
                ...
                <tbody>
                    <tr>
                        <td width="50%">
                            ...
                            <br>
                            <div><b>{{__('messages.fullname')}} : {{$delivery_order->supplier->fullname}}</b></div>
                            <div><b>{{__('messages.email')}} : </b>{{$delivery_order->supplier->email}}</div>
                            <div><b>{{__('messages.address')}} : </b>{{$delivery_order->supplier->address}}</div>
                            <div><b>{{__('messages.phone')}} : </b>{{$delivery_order->supplier->email}}</div>
                        </td>
                        <td width="15%" class="text-verticaly-top">
                            <div><div><b>{{__('messages.date')}} : </b>{{\Carbon\Carbon::parse($delivery_order->start_date)->format('d/m/Y')}}</div></div>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div class="div-table">
            <table class="table-sale-invoice">
                ...
                <tbody>
                    @foreach($delivery_order->delivery_order_lines as $line)
                        <tr>
                            <td>
                ...
                <tfoot>
                    <tr>
                        ...
                        <td>{{$delivery_order->ht_total_amount}} <b>MAD</b></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        ...
                        <td>{{$delivery_order->tva_total_amount}} <b>MAD</b></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        ...
                        <td class="text-white bg-black">{{$delivery_order->ttc_total_amount}} <b>MAD</b></td>
                    </tr>
                </tfoot>
            </table>
        </div>
