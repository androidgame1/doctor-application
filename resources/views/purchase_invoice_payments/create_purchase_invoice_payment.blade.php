...
<div class="row page-titles">
    <div class="col-12 align-self-center">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a
                        href="@if(auth()->user()->is_administrator){{route('administrator.purchase_invoice_payments',$purchase_invoice->id)}}@else javascript:void(0) @endif">{{__('messages.purchase_invoice_payments')}}</a>
                </li>
                <li class="breadcrumb-item active">{{__('messages.new_purchase_invoice_payment')}}</li>
            </ol>
    </div>
</div>
<!-- ============================================================== -->
...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <form method="post" id="form-create-new-purchase-invoice-payment" action="@if(auth()->user()->is_administrator){{route('administrator.purchase_invoice_payment.store')}} @else javascript:void(0) @endif" enctype="multipart/form-data" class="needs-validation" novalidate>
                    @method('post')
                    @csrf
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="number" value="{{$purchase_invoice->remaining_amount}}" min="0.1" max="{{$purchase_invoice->remaining_amount}}" step="0.01" class="form-control {{$errors->has('given_amount')?'form-control-danger':''}}" placeholder="{{__('messages.given_amount')}}" name="given_amount" required>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
