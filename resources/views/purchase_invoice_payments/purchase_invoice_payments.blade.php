        ...
        <div class="card">
            <div class="card-body">
                <div class="row">
                    ...
                    @include('purchase_invoice_payments.payment_to')
                    <div class="col-md-12">
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped table-datatable">
                                ...
                                <tbody>
                                    @foreach($purchase_invoice_payments as $value)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <!-- <a href="@if(auth()->user()->is_administrator){{route('administrator.purchase_invoice_payment.edit',$value->id)}}@else javascript:void(0) @endif" title="{{__('messages.edit')}}"> <i class="fa fa-pencil text-success m-r-10 icon-datatable"></i> </a> -->
                                                <a href="javascript:void(0)" class="btn-destroy-item" data-toggle="modal" data-target="#div-destroy-old-item" data-url-destroy="@if(auth()->user()->is_administrator){{route('administrator.purchase_invoice_payment.destroy',$value->id)}}@else javascript:void(0) @endif" data-title="{{__('messages.delete_purchase_invoice_payment')}}" data-message="{{__('messages.do_you_want_to_delete_this')}} {{__('messages.purchase_invoice_payment')}} ?" title="{{__('messages.delete')}}"> <i class="fa fa-trash text-danger icon-datatable"></i> </a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                                <tfoot>
                                    <tr>
                                        ...
                                        <td colspan="6"><span class="text-success font-bold">{{$purchase_invoice->given_total_amount}} <b>MAD</b></span></td>
                                    </tr>
                                    <tr>
                                        <td><b>{{__('messages.total_remaining_amount')}}</b></td>
                                        <td colspan="6"><span class="text-danger font-bold">{{$purchase_invoice->remaining_total_amount}} <b>MAD</b></span></td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
...
@include('modals.purchase_invoice_payments.show_purchase_invoice_payment')
...
@include('javascript.purchase_invoice_payments.show_purchase_invoice_payment')
