        ...
        <div class="d-flex justify-content-end align-items-center">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a
                        href="@if(auth()->user()->is_administrator){{route('administrator.purchase_invoices')}}@else javascript:void(0) @endif">{{__('messages.dashboard')}}</a>
                </li>
                                ...
                                <tbody>
                                    @foreach($purchase_invoices as $value)
                                        <tr>
                                            ...
                                            <td><a href="javascript:void(0)" class="btn-show-purchase-invoice" data-toggle="modal" data-target="#div-show-old-patient" data-url-show="@if(auth()->user()->is_administrator){{route('administrator.purchase_invoice.show',$value->id)}}@else javascript:void(0) @endif"  data-original-title="{{__('messages.show')}}">{{$value->series}}</a></td>
                                            ...
                                            <td>
                                                <a href="@if(auth()->user()->is_administrator){{route('administrator.purchase_invoice.show',$value->id)}}@else javascript:void(0) @endif" data-toggle="tooltip" data-original-title="{{__('messages.show')}}"> <i class="fa fa-eye text-info m-r-10 icon-datatable"></i> </a>
                                                <a href="@if(auth()->user()->is_administrator){{route('administrator.purchase_invoice.edit',$value->id)}}@else javascript:void(0) @endif" data-toggle="tooltip" data-original-title="{{__('messages.edit')}}"> <i class="fa fa-pencil text-success m-r-10 icon-datatable"></i> </a>
                                                <a href="@if(auth()->user()->is_administrator){{route('administrator.purchase_invoice.duplicate',$value->id)}}@else javascript:void(0) @endif" data-toggle="tooltip" data-original-title="{{__('messages.duplicate')}}"> <i class="fa fa-files-o text-warning m-r-10 icon-datatable"></i> </a>
                                                @if($value->status == '0')
                                                    <a href="javascript:void(0)" class="btn-cancel-item" data-toggle="modal" data-target="#div-cancel-old-item" data-url-cancel="@if(auth()->user()->is_administrator){{route('administrator.purchase_invoice.cancel',$value->id)}}@else javascript:void(0) @endif" data-title="{{__('messages.purchase_invoice')}}" data-message="{{__('messages.do_you_want_to_cancel_this')}} {{__('messages.purchase_invoice')}} ?" data-toggle="tooltip" data-original-title="{{__('messages.destroy')}}"> <i class="fa fa-ban text-danger icon-datatable"></i> </a>
                                                @endif
