...
<div class="row page-titles">
    <div class="col-md-5 align-self-center">
        <h4 class="text-themecolor">{{__('messages.edit_quote')}}</h4>
    </div>
    <div class="col-md-7 align-self-center text-right">
        <div class="d-flex justify-content-end align-items-center">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">{{__('messages.edit_quote')}}</li>
            </ol>
        </div>
    </div>
</div>
...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <form method="post" id="form-edit-quote" action="@if(auth()->user()->is_administrator){{route('administrator.quote.update',$quote->id)}} @else javascript:void(0) @endif" enctype="multipart/form-data" class="needs-validation" novalidate>
                    @method('put')
                    @csrf
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control {{$errors->has('patient_id')?'form-control-danger':''}}"
                                    ...
                                    @foreach($patients as $value)
                                        <option value="{{$value->id}}" {{$value->id == $quote->patient_id ? 'selected' : ''}}>{{$value->fullname}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-md-6 col-12">
                            <div class="form-group">
                                ...
                                <input class="form-control {{$errors->has('series')?'form-control-danger':''}}"
                                    type="text" placeholder="{{__('messages.series')}}" value="{{$quote->series}}" name="series" readonly required>
                            </div>
                        </div>
                        <div class="col-md-6 col-12">
                            <div class="form-group">
                                ...
                                <input class="form-control {{$errors->has('date')?'form-control-danger':''}}"
                                    type="date" placeholder="{{__('messages.date')}}" name="date" value="{{\Carbon\Carbon::parse($quote->date)->format('Y-m-d')}}" required>
                            </div>
                        </div>
                        <div class="col-md-12">
                            <div class="form-group">
                                <label class="label-group">{{__('messages.remark')}}<span class="text-danger d-none"> * </span></label>
                                <textarea rows="4" class="form-control {{$errors->has('remark')?'form-control-danger':''}}" placeholder="{{__('messages.remark')}}" name="remark">{{$quote->remark}}</textarea>
                            </div>
                        </div>
                        <div class="col-md-12">
                            <div class="form-group">
