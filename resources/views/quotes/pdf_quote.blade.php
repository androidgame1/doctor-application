...
<body>
    <header class="header">
        ...
        <div class="div-header-title">
            <span class="header-title">{{__('messages.quote')}} : <span class="span-series">N° {{$quote->series}}</span></span>
        </div>
    </header>
    <footer class="footer">
    ...
    <main>
                ...
                <tbody>
                    <tr>
                        <td width="50%">
                            ...
                            <br>
                            <div><b>{{__('messages.fullname')}} : {{$quote->patient->fullname}}</b></div>
                            <div><b>{{__('messages.email')}} : </b>{{$quote->patient->email}}</div>
                            <div><b>{{__('messages.address')}} : </b>{{$quote->patient->address}}</div>
                            <div><b>{{__('messages.phone')}} : </b>{{$quote->patient->email}}</div>
                        </td>
                        <td width="15%" class="text-verticaly-top">
                            <div><div><b>{{__('messages.date')}} : </b>{{\Carbon\Carbon::parse($quote->start_date)->format('d/m/Y')}}</div></div>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div class="div-table">
            <table class="table-sale-invoice">
                ...
                <tbody>
                    @foreach($quote->quote_lines as $line)
                        <tr>
                            <td>
